# Домашнее задание 1
## Татьяна Авдеева
Поиск для данных с сайта [библиографии исторического танца](https://bib.hda.org.ru).

В качестве корпуса — метаданные источников, содержащих знания о танцах. Я выбрала этот корпус, потому что занимаюсь историей танца, и мне хотелось бы помочь реально существующему проекту.

На данный момент поиск реализован скорее для русского (задействована мультиязычная [модель](https://github.com/adbar/simplemma), которую легко адаптировать для других языков).

Я надеюсь, что я всё нужное выгрузила, но если вдруг что, изначально я работала с клонированным [открытым репозиторием](https://github.com/hda-technical/dancebooks) сайта библиографии.
## Как запускать

В файле [infopoisk_search.py](https://github.com/singing2owl/infopoisk2026/blob/main/infopoisk_search.py) реализован частотный и bm25-поиск с помощью словарей. Можно запустить программу и искать с помощью стандартного ввода.

В файле [infopoisk_search_matrix.py](https://github.com/singing2owl/infopoisk2026/blob/main/infopoisk_search_matrix.py) всё так же, только поиск реализован с помощью матриц.

В обоих файлах используется код из соседних файлов репозитория.

В выдаче видны идентификаторы источников. При желании посмотреть, что это за источники, можно вбивать их в обычный поиск, уже реализованный на [сайте](https://bib.hda.org.ru).
